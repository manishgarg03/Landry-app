...
//   Hive.box(AppHSC.authBox)
//       .put('notification_switch_state', isPermissionGranted);
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  final notificationBox = Hive.box(AppHSC.appSettingsBox);
  final notificationSwitchState = notificationBox.get(
    'notification_switch_state',
    defaultValue: true,
  );
  if (notificationSwitchState == false) return;
  await setupFlutterNotifications();
  showFlutterNotification(message);
  debugPrint('Handling a background message ${message.messageId}');
}
...
void handleMessage(RemoteMessage? message) {
  if (message == null) return;
  if (message.data['type'] == 'Conversetion') {
    // Handle message-specific navigation
  }
}
...
Future<void> setupFlutterNotifications() async {
  ...
  var as = Hive.box(AppHSC.appSettingsBox).get('notification_switch_state');
  print("isPermissionGranted $as");
...
void main() async {
  ...
  // Listen to the notification toggle switch value
  final notificationSwitchState = Hive.box(AppHSC.appSettingsBox).get(
    'notification_switch_state',
    defaultValue: true,
  );

  // If the switch is ON, register for notifications, else stop notifications
  if (notificationSwitchState == true && token != null) {
    // Register for notifications and send token to backend if required
    debugPrint(
        "Notifications are ON. Registering for notifications with token.");
    // Example: sendTokenToServer(token);
  } else if (notificationSwitchState == false && token != null) {
    // Unsubscribe from notifications or stop using token when notifications are OFF
    debugPrint("Notifications are OFF. Unsubscribing from notifications.");
    // Example: unsubscribeFromNotifications(token);
  }
